    ...
    return (
        <>
            <div className="flex flex-col w-full items-center justify-start">
                ...
                <div className={cn("w-full transition-all overflow-hidden ease-linear duration-800", nodeListOpened ? "max-h-auto mb-4" : "max-h-0 mb-0")}>
                    {files.map((file, index) => {
                        if (getFile(file.path).extension === targetExtension) {
                            return <FileNode key={index} filePath={file.path} />
                        }
                    })}
                </div>
            </div>
    ...
    return (
        <>
            <div className="h-full flex flex-col items-start justify-start w-[230px] bg-sidebar border-solid border-b-[1px] border-r-[1px] border-border pt-2 gap-2">
                <p className="ml-6 mt-2 text-neutral-300 text-[14px] font-semibold">cubane</p>
